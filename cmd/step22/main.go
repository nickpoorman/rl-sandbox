...
// One of the challenges with using neural networks for Q-learning is that the
// network is constantly changing as the agent learns, leading to instability
// and divergence. To address this, we can introduce a target network, a concept
// popularized by the Deep Q-Network (DQN) algorithm. The target network is a
// separate neural network used to generate the target Q-values during training,
// and it is periodically updated to the current Q-network.
...
func (agent *Agent) UpdateQValues(state, action int, reward float64, newState int) {
	...
	agent.Model.Update(input, targetOutput, LearningRate) // learningRate of 0.01
