...
func (agent *Agent) UpdateQValues(state, action int, reward float64, newState int, done bool) {
	...
	bestAction := agent.Model.BestAction(newState) // Best action using the main Q network

	// Use the target network to estimate the Q-value of the best action
	nextInput := make([]float64, agent.TargetModel.NumInput)
	nextInput[newState] = 1
	nextQValues := agent.TargetModel.Forward(nextInput)
	...
	targetOutput[action] = normalizedReward
	if !done {
		targetOutput[action] += Gamma * nextQValues[bestAction]
	}
...
func (nn *NeuralNet) BestAction(state int) int {
	input := make([]float64, nn.NumInput)
	input[state] = 1
	qValues := nn.Forward(input)

	maxAction := 0
	maxQValue := qValues[0]
	for a := 1; a < len(qValues); a++ {
		if qValues[a] > maxQValue {
			maxAction = a
			maxQValue = qValues[a]
		}
	}
