...
// We'll use Neural Networks as function approximators, leveraging the powerful
// capabilities of Deep Reinforcement Learning. We'll introduce a simple
// feedforward neural network as our Q-function approximator.
...
func (agent *Agent) UpdateQValues(state, action int, reward float64, newState int) {
	...
	if newState != -1 {
		nextInput := make([]float64, agent.Model.NumInput)
		nextInput[newState] = 1
		nextQValues := agent.Model.Forward(nextInput)

		// Assuming a discount factor (Gamma) of 0.9
		targetOutput[action] += 0.9 * max(nextQValues)
	}

	agent.Model.Update(input, targetOutput, 0.01) // learningRate of 0.01
